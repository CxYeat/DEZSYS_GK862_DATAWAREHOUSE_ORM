...
This is the core "Deep Dive" functional extension of the application. The system integrates **Google Gemini** to support business intelligence and data-driven decision-making.

- **Data Aggregation:** The service retrieves all historical purchase records from the MySQL database using Spring Data JPA. It utilizes **Java Streams** to aggregate sales figures, grouping them by product name and warehouse location to provide a comprehensive dataset for the AI.

- **Prompt Engineering:** The system automatically constructs a structured prompt. This prompt includes the aggregated sales data and explicit instructions for the AI to act as a retail analyst, ensuring the output is professional and formatted correctly.

- **Cloud AI Integration:** Communication is handled via Spring's `RestTemplate`. The service performs a POST request to the **Google Gemini API**, passing the API key as a secure credential and the aggregated data in the required JSON body format.

- **Predictive Analysis:** The AI evaluates the provided historical data to generate a **3-month sales forecast**. It identifies potential demand peaks and provides a trend analysis to assist in inventory optimization across different warehouse locations.

### 4. REST API for Real-Time Forecasts

The forecasting logic is exposed through a specialized REST Controller, allowing external systems or frontends to trigger analysis on demand.

**Endpoint:** `GET /api/v1/forecast/generate`

- **Controller Layer:** A `@RestController` handles incoming HTTP requests and invokes the `ForecastService`.

- **Service Layer:** The `ForecastService` performs the heavy lifting—fetching data, communicating with Google Gemini, and parsing the complex JSON response.

- **Response Format:** The API returns a JSON object containing the status, the model used, and the raw markdown text generated by the AI, making it easy to display in a web dashboard.

![](C:\Users\aronh\AppData\Roaming\marktext\images\2026-03-23-20-40-22-image.png)
